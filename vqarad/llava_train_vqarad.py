...
class VQARADScore(Metric):
    def __init__(self, dist_sync_on_step=False):
        ...
        self.add_state("close_correct", default=torch.tensor(0.0), dist_reduce_fx="sum")  # Accuracy for closed-set
        self.add_state("close_total", default=torch.tensor(0.0), dist_reduce_fx="sum")

        self.add_state("open_recall", default=torch.tensor(0.0), dist_reduce_fx="sum")  # Recall for open-set
        self.add_state("open_total", default=torch.tensor(0.0), dist_reduce_fx="sum")  # Total tokens in targets

        # Track best scores
        self.best_score = 0.0
        self.best_close_score = 0.0
        self.best_open_recall = 0.0
    ...
    def update(self, preds, targets, types=None):
        ...
        if types is not None:
            for pred, target in zip(preds, targets):
                if types == 0:  # Close-ended (Accuracy)
                    self.close_total += 1
                    if target in pred:
                        self.close_correct += 1
                elif types == 1:  # Open-ended (Recall)
                    target_tokens = target.split()
                    pred_tokens = pred.split()
                    # Create binary arrays for recall calculation
                    y_true = [1] * len(target_tokens)  # All target tokens are relevant
                    y_pred = [1 if token in pred_tokens else 0 for token in target_tokens]  # Check if token is present
                    recall = recall_score(y_true, y_pred, zero_division=0)
                    self.open_recall += recall * len(target_tokens)  # Accumulate weighted recall
                    self.open_total += len(target_tokens)

    def compute(self):
        """Compute the final overall metric value (combined)."""
    ...
    def get_best_score(self):
        ...
        if score > self.best_score:
            ...
            self.best_open_recall = (self.open_recall / self.open_total).item() if self.open_total > 0 else 0.0
        return self.best_score

    def get_best_close_score(self):
        """Get the best close-ended score (accuracy)."""
        return self.best_close_score

    def get_best_open_recall(self):
