...
class VQARADScore(Metric):
    ...
    def update_best_scores(self):
        """Update the best scores if the current scores are better."""
        current_score = self.compute()
        if current_score > self.best_score:
            self.best_score = current_score.item()
            self.best_close_score = (
                (self.close_score / self.close_total).item() if self.close_total > 0 else 0.0
            )
            self.best_open_score = (
                (self.open_score / self.open_total).item() if self.open_total > 0 else 0.0
            )
...
def main():
    ...
    # Save final model
    model.model.save_pretrained("llava-finetuned-vqarad-10")
    processor.save_pretrained("llava-finetuned-vqarad-10")
